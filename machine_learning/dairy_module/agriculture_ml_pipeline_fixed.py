"""
Comprehensive Agricultural Dataset Training Pipeline - Fixed Version
...
from sklearn.ensemble import RandomForestRegressor, RandomForestClassifier, GradientBoostingRegressor, HistGradientBoostingClassifier, HistGradientBoostingRegressor
...
class AgricultureMLPipelineFixed:
    """Comprehensive ML pipeline for agriculture dataset - Fixed version"""
    ...
    def __init__(self, data_path="data/agriculture/Production_Crops_Livestock_E_All_Data.csv"):
        ...
        # Create directories
        self.model_dir = f"models/agriculture_fixed_{self.timestamp}"
        self.report_dir = f"reports/agriculture_fixed_{self.timestamp}"
        self.vis_dir = f"visualizations/agriculture_fixed_{self.timestamp}"
    ...
    def preprocess_data(self):
        ...
        if production_df.empty:
            # If 'Production' isn't found, use all data
    ...
    def train_regression_models(self):
        ...
        y = features_df['mean_production'].copy()
        
        # Handle missing values
        X = X.fillna(X.mean())  # Fill missing values in features
        y = y.dropna()  # Drop missing values in target
    ...
    def train_classification_models(self):
        ...
        print("\n=== Training Classification Models - FIXED ===")
        
        features_df = self.processed_data['features_df']
        
        # Create target variable: classify production level (low, medium, high)
        features_df_clean = features_df.dropna(subset=['mean_production'])
        production_thresholds = np.percentile(features_df_clean['mean_production'], [33, 66])
        ...
        features_df_clean['production_class'] = features_df_clean['mean_production'].apply(classify_production)
        
        # Prepare features and target
        feature_cols = ['Area_encoded', 'Item_encoded', 'std_production', 'min_production', 'max_production']
        X = features_df_clean[feature_cols].copy()
        y = features_df_clean['production_class'].copy()
        ...
        # Define classification models (avoiding ones that don't handle missing values)
        clf_models = {
            'HistGradientBoosting Classifier': HistGradientBoostingClassifier(random_state=42),
    ...
    def run_complete_pipeline(self):
        ...
        print("=" * 70)
        print("AGRICULTURE DATASET ML PIPELINE - ALL POSSIBLE APPROACHES (FIXED)")
        print("=" * 70)
...
if __name__ == "__main__":
    ...
    pipeline = AgricultureMLPipelineFixed()
